...
（如果是填空题，挖空部分请统一使用 \`____\` 四个连续下划线表示占位）
...
（【重要】若该题存在多种解题方法，不同解法之间必须使用 --- 独占一行进行分隔，如：
解法一的内容
---
解法二的内容）

## 属性
题型: 单选题 或 多选题 或 填空题 或 解答题 或 判断题（必填其一）
...
4. 不要添加代码块标记（如 \`\`\`markdown）
5. ## 答案 只写最终结果，所有推导过程一律放到 ## 解析 中，不要混到答案里
6. ## 解析 中多小题必须用 ### (1) 等分隔；多解法必须用 --- 分隔
7. 块级公式 $$...$$ 必须独占一行，且公式行前后不要留空行（即公式与上下文紧邻）
8. 段落之间用单个空行分隔即可，不要连续多个空行
9. ## 属性 部分的字段各占一行，格式为"字段名: 值"
10. 【缺失信息处理】：如果提供的图片中仅有题目，没有答案和解析，你必须严格保留 \`## 答案\` 和 \`## 解析\` 这两个标题，并在标题下方直接输出“暂无”两字。绝对禁止自行编造或推导答案，也严禁省略标题！`
