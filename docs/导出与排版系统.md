...
### FROZEN: Word 格式功能缺口（UI 已冻结，后端保留）

> Word 导出已在前端隐藏（`DOCX_ENABLED = false`），以下缺口在解冻前不纳入迭代，仅做记录。

| 缺口 | 说明 | 解冻时优先级 |
| --- | --- | --- |
| 密封线 | DOCX 无原生侧栏密封线，需用 DrawingML 竖排文字模拟 | P1 |
| 动态页眉 | Word 不支持跨大题自动换页眉文字；需 SectionBreak + 手动设 header | P1 |
| 留白样式 | 横线格/点阵（dots）无 OOXML 原生对应，需 DrawingML 画线/圆 | P2 |
| K100 纯黑 | Word 无全局色彩模式开关；需逐 run 设 `w:color` + 图片灰度化 | P2 |
| 左文右图 | figure_float 在 DOCX 侧未实现；退化为上下排列 | P3 |
...
4. **~~估宽口径差~~（已修复）**：`HANGING_EM` 与 `HANG_EM` 已统一为 `choice_grid::QUESTION_INDENT_EM = 2.6`，两模块共用同一常量
5. **复杂选项单元格收缩**：包含图片/表格/硬换行/块级公式的选项（`boxed: false` 分支）走 Typst `grid` 渲染时会收缩为内容宽度而非等宽铺满（follow-up #38）
6. **预览 SVG 不可选中**：Typst 编译 SVG 使用 `<path>` 描边而非 `<text>` 节点，文字无法选中/搜索。这是 Typst SVG 后端的已知限制。如需复制文字，可从源数据获取（TypesetPreview 工具栏可扩展 copy 按钮）
