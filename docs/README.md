...
| 文档 | 说明 |
|------|------|
| [结构化排版：最终稿](结构化排版_最终稿.md) | **唯一排期口径**：冻结非排版；PublicationDocument；`/studio`；双层模板；编译 API 契约；往返一致性；档 B 视觉单源（未启用） |
| [结构化排版：主线与冻结](结构化排版_主线与冻结.md) | （已被最终稿取代，保留备查）冻结非排版；PublicationDocument；`/studio`；双层模板；HTML↔Typst 一致；长文档虚拟化 |
| [结构化排版：档 B Typst 视觉单源与热区](结构化排版_档B_Typst视觉单源与热区.md) | （已并入最终稿 §六，保留备查）视觉单源路径：仅 Typst 皮肤 + 当前页 SVG 热区 |
| [导出与排版系统](导出与排版系统.md) | Typst 导出/预览能力说明 |
