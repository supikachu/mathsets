...
## 2026-09-02 导出引擎 M4 批次④（A3 对折逻辑页与密封线）

### 一、⛔ 脚本层问不到「现在是第几栏」：号只能在 Rust 侧算

这是 T4.7 那道门的全部难度。typst 的 `footer` **每物理页只排一次**，而且是按整个版心宽排的（实测
typst-layout `pages/run.rs` 用合并后的页框宽算 marginal、`pages/finalize.rs` 只把它挂在外层页框
上）；「此刻排到第几栏」只是 `flow/compose.rs` 里的一个循环变量，脚本层没有任何出口。所以「A3 对折
左半 = 第 X 页、右半 = 第 X + 1 页」在 typst 里**没有对应原语**，`numbering` / `number-align` 那两条
规则更是死的 —— 给了自定义 footer 就把内置编号整个关掉（R4 已实测）。

能做的只有自己在页脚里切格，并把号算成表达式：

```typst
#set page(footer: context {
  let p = here().page(); let t = counter(page).final().first();
  grid(columns: (1fr, 1fr), gutter: (20mm, 0pt),
    align(left + horizon, [#page-cell(2 * p - 1, 2 * t)]),
    align(right + horizon, [#page-cell(2 * p, 2 * t)]))
})
```

三条口径都是这里定死的：格的 `columns` / `gutter` 与正文栏**同值**，于是每格正好落在自己那一栏正下
方；`here().page()` 与 `counter(page).final()` 给的都是**物理纸张数**，恰好是乘格数的底数；`共 Y 页`
取 `格数 × 纸张数`，最后一张只用半张时也会多报一页 —— 那半张确实存在、只是空着，与送印厂给的页数
口径一致。号写成 `2 * p - 1` 而不是 `2 * (p - 1) + 1`，是常数项合并的结果，少一对括号少一次歧义。

### 二、`#set page(…)` 的参数在写它的那一刻求值：页脚与装订带进不了函数库

第一版把 `page-cell` / `sealing-line` 放进了 `FUNCTION_LIBRARY`，实测直接报
`unknown variable: page-cell` —— 函数库整段排在母版**之后**（正文要用它），而母版的
`footer: context { … }` 在写它那一刻就要拿到这两枚函数。顺序上只能挑一头，于是它们单独成了
`PAGE_FURNITURE` 常量，赶在母版前落地。

代价记在常量自己的 doc 里：这两枚不许引用母版变量（`accent` / `body-font` 那四个此刻还不存在），
颜色与字号只能写死 `luma(110)` / `9pt`。页脚与装订带本来就是全卷一套、不跟着 spec 漂，这笔交换划
得来。

### 三、typst 没有 `%` 运算符

双面外侧判定第一版写成 `p % 2 == 0`，编译报错是一次三连同发：

```
["the character `%` is not valid in code", "expected comma", "unexpected keyword `else`"]
```

**别照着最后一条找问题** —— 一条式子错，typst 会顺着往后撞坏两三个 token 再收工。改 `calc.even(p)`
后即通。

### 四、⚠ `rotate` 默认绕框中心转：整条装订带飞出页面

密封带第一次编译出来是「看不见」。帧树里量到的落点是 x≈277.8、y≈−63.7，而 `place` 写的是
(192, 18) —— 差值正好是那行字的半宽与半高：`rotate` 的 `origin` 默认是 **center**，锚点被甩到文字框
中心去了，压上右栏正文并越出页顶。
...
左上角为原点 + `reflow: false`（默认）才是「锚点 = 带子右上角、向下长出」，于是模板参数只需要一个
`x`。修完复测 x=208.0 / y=20.1，与线位（折痕左侧 2mm）吻合。

两件事要一起记住：

- **帧树对旋转过的内容报的是摆放锚点，不是它印在纸上的那条包围盒**。`compiler.rs` 的坐标口径 doc 已
  写明这点（生成的源码里没有 `rotate` 之外的变换，装订带是唯一例外）。想问「带子有没有压上正文」，
  只能反过来看这枚 run 的 x 落在哪个区间。
- 旋转前 CJK 就会先折行，所以**行的自然宽度必须小于页高**。长度归 Rust 侧管（`Gen::sealing` 按字段
  数拼 label），折行守卫则由验收用例代劳：「分栏之间那条 x 带里只许存在一枚 run」。

### 五、装订带走 page `background`，不走正文流

背景帧**每页一枚、与页面等大、按绝对坐标摆放**（实测 `pages/finalize.rs` 用 `full_size` 排
background 并挂在 `bleed_origin` = 页面左上角），于是一条 20mm 的带子既不占版心也不进分栏流动。
另一条路是让带子作为正文流的一部分排在栏外——`columns` 容器里 `pagebreak` 会直接报错（见
`Gen::prologue` 里「栏数走页级」那段实测），栏内也没有「跨出栏宽」这个概念。除了背景没有第二条路。

### 六、几何只有一处真相：`SEALING_BAND_MM` 住在 spec.rs

两种装订位共用同一枚模板函数，差别全在参数上，而参数来自两条派生函数：

| 装订位 | 生效几何 | 线位 x |
| --- | --- | --- |
| `Left` | `margin_left_mm() = left_mm + 带宽` → 正文整块右移 | `带宽 − 2mm`（虚线贴带子右沿） |
| `CenterFold` | `column_gutter_mm()` 兜到不低于带宽 → 挤瘦两栏 | `左边距 + 栏宽 + 栏距 / 2`（折痕） |

两者都汇入 `column_width_mm()`，于是母版、选项栅格、题块估高、页脚格宽、装订线 x 全从同一条算式来，
不可能各改一半。折痕位置是**算出来的**而非常量：a3_fold_exam = 16 + 184 + 20/2 = 210mm，恰为
420/2 —— 用例里就断这个相等，几何漂了立刻红。

外侧对齐在这里有两套语义，方向**相反**：

- 单栏双面：奇数页是正面、外沿在右，偶数页是背面、外沿在左（翻面换边）。
- 对折多格：折痕在纸中央，所以左半页的外沿是纸的**左**沿、右半页是右沿，且一张纸上的两格不会随翻面
  对调；三栏那档的中栏没有外沿可言，留在格心。

### 七、⛔ 门怎么算过：把「页码对不对」变成帧树里可读的东西

验收三条（双栏自然流动 / 逻辑页计数 / 奇偶外侧）全部读编译产物，眼睛那份只做人工复核。
`footer_cells(page)` 从帧树明文里剥出 `第 N 页 / 共 M 页` 并按 x 排序，于是三张 A3 纸报的就是
1/2、3/4、5/6 且 `共 6 页`；两条对齐断言故意不硬编码 x，只问「左格 == 版心左缘」「右格距右缘
< 30mm」，再加一句「y 在内容底缘之下」证明它真是页脚。流动顺序用 `spot(needle, column_x)`：拿 run
的 x 落在第几栏，判「第 1 题在第 1 格、第 20 题在更靠后的格」。栏数与纸张打架时另有
`warn_page_conflicts` 出一条 `Structure` Warning（「会与折痕错位」），2 栏预设保持静默。

`#[ignore] writes_fold_and_seal_samples` 出 SVG，再用 headless Chrome 栅成 PNG 目视：折痕虚线落在页面
正中，带上的「学校 / 班级 / 姓名 / 考号 / 密封装订线，请勿折叠」竖排在分栏带内不碰两栏正文，两个页号
分居底缘两角。

### 八、本批次已知边界

- 页脚只做了页码。动态页眉取当前大题名是 T4.10；每页都画装订带，首页母版分离在 T4.9。
- A3 三栏的 `logical_slots_per_sheet()` 是 **1** —— 那里的栏只是栏，不多造页码。哪天要按栏编号，改
  这一个 match 就够（`LayoutSpec::logical_slots()` 会跟着走）。
- `logical_slots()` 取的是「纸张口径与实际栏数的较小值」：A3 对折排成单栏时「半张 = 一页」不成立，
  与其编出对不上折痕的页号，不如老实只编一个（外加第七条那条 Warning）。
- 密封带上的考号是**画出来的下划线**，真实考卷的条码区在 T4.9 的完整卷头里。
- 装订带按「右上角」锚定，前提是这个 x 左侧有 20mm 可长。将来若加右侧装订位，得另给一枚 x 口径。
- 样卷 PNG 不进 CI（一次四编 + 落盘）。常驻的是读帧树的那五条。

## 2026-09-03 导出引擎 M4 批次⑤a（首页大卷头与母版分离）

### 一、⛔ typst 0.15 没有 `page(first:)`：母版分离只能走容器

任务表把 T4.9 写成「`page(first:)` 参数分离」，那是 ≤0.13 的参数。读 vendored
`typst-library-0.15.1/src/layout/page.rs` 的字段表，里面没有 `first`；剩下的两条路各自实测（探针
`masters_separation_probe` 的 E1–E6，跑法在它的 doc 行里）：

- **flow 中途 `#set page(columns: 2)`**：生效点是**下一页**，不是本页（E1）。
- **`#page(columns: 1)[body]` 作用域**：另起一页，且块结束后回落到外层母版（E2）。
- **页级单栏 + `#columns(n, gutter)[…]` 容器**：通栏卷头块与双栏正文**同张共存**（E3，120×40mm
  页面上第 1 张同时量到 x=5.0 与 x=63.0 的落点，还带着那块通栏），容器跨页时每一张仍按自己的栏数
  分栏，容器结束后正文回到页级单栏宽。

只有第三条能用，而它的前置条件是 `#pagebreak` **留在容器之外** —— 容器内直接报
`pagebreaks are not allowed inside of containers`（E4）。于是卷面形状被钉死成「通栏卷头 → 正文容器
→ 页级 `#pagebreak(weak: true)` → 答案区容器」，这段仲裁写进了实施计划 R8 再动代码。

落地后页级 `columns` 恒为 1，栏数与栏距从 `#set columns` 迁进容器参数（`Gen::in_columns`），单栏不
包壳（容器与页级单栏排出来一模一样，多一层壳只是给 `pagebreak` 添雷）。栏距仍取
`column_gutter_mm()`，与页脚切格同一个数，R4 那套逻辑页码账不受影响。

### 二、通栏卷头与折痕上的密封带，二者只能挑一个

卷头一通栏，A3 对折卷的第 1 张就变成「整幅宽横穿 x=210 的折痕」：分值表的中缝格线正好压在 20mm
密封带上，装订吃掉一格。更要紧的是这张整幅卷头页在 R4 的逻辑页口径下算 1 页还是 2 页无解 —— 等于
推翻上一批次已经过的门。

真实对折卷的卷头本来就只占半张（第 1 版），所以仲裁取「折痕竖在版心里时卷头进首栏」，写成
`Gen::seal_crosses_content()`：`CenterFold` + 多栏 → 卷头进容器；`None` / `Left` → 通栏。两条都读
帧树断，不靠源码字符串：a4_practice 上「合计」的 x 必须越过右栏左沿（真通栏），a3_fold 上「合计」
必须留在首栏右沿以里（没压折痕），且第 2 张不许再出现「合计」（母版真分开了）。

### 三、一张卷子只许出现一个姓名栏

开了装订位时，学校/班级/姓名/考号已经竖排在密封带上，首页再横排一遍就是同一份卷子上两个姓名栏。
按「已仲裁分歧③」处理：`Gen::candidate_info()` 返回 `Option`，有 binding 即 `None`。用例两头都断
—— 装订卷源码里不许出现 `info: ("`，无装订卷必须出现 `info: ("学校：示例中学"`（DOCX 没有带子能
力，退化的正是这张表，所以那边恒有）。

题号键与 docx 同源：`section_key` 从 `export/docx/writer.rs` 提成 `pub(crate)` 给 PDF 侧复用。同一
份 IR 的两种格式，分值表第一行不许一边是「一、二、三」一边是「1、2、3」。

### 四、0.15 的 `table` 没有 `width`，auto 列还会收缩

想让分值表铺满，第一反应是 `width: 100%` —— grep `model/table.rs`，`TableElem` 字段表里没有
`width`。去掉后表确实排出来了，但帧树里「合计」只到 x=52.2：**auto 列按内容收缩**，整张表只有内容
那么宽。改成 `columns: range(n).map(_ => 1fr)` 才真铺满，这条写进 `header-table` 的注释。

### 五、⚠ 卷头表格的 0.4pt 边框，污染了两处「数横线」的口径

加卷头之前，「纸上出现的通宽横线」等价于「留白真画出来了」；加完之后讲义从 0 条变 6 条（信息表上
下 2 条 + 三行分值表 4 条），`the_three_modes_differ_exactly_as_the_switches_say` 红在「讲义不留
白、答案也不卷末汇总」。留白与密封线是 ≥0.5pt，表格边框是 0.4pt，于是两边同一枚门槛
`RULE_MIN_MM = 0.5pt − 0.01mm`（typst_gen 的 `drawn_rules` 与 pdf.rs 的 `Paper::rules`），并把口径
写进 doc：`rules` 数的是留白与密封线，不是全卷所有横线。这类「代理指标被新功能顺手喂胖」的账，每
加一种画线的东西都要回头看一眼。

### 六、门在纸面上：run 是按折行切的

`placed_pages` 给的是**每一折行一枚 run**，所以「第 1 题」这种跨行 needle 在窄栏里永远搜不到 ——
卷头那条断言的 needle 换成「一、单项选择题」（大题标题不会折行）。页眉也没停在源码字符串上：
`first_sheet_has_no_running_header` 读帧树，`y < margin.top` 就是页眉条与版心的分界，第 1 张的卷头
名必须恰好一枚且落在版心里（真由 masthead 画的），第 2 张必须有一枚浮到上沿之上（条件页眉真顶上
来了）。

### 七、本批次已知边界

- 页眉内容仍是 `doc.title` 静态串，逐页取当前大题名是 T4.10；`header_footer.header_title` 默认
  false，所以三模式样卷上看不到页眉（不是漏画）。
- `#[ignore] masters_separation_probe`（E1–E6）留在测试末尾当 R8 的实测凭据，不进常驻门。
- 密封带上的考号仍是画出来的下划线，真实考卷的条码区没做。
- 卷头三张表在 docx 侧是 twips 定宽、在 PDF 侧是 `1fr`：同一份数据两种栅格法，都保证铺满，不保证
  逐格等宽。
- 页脚与装订带的灰阶（`luma(110)` / `luma(140)` / `luma(90)`）还不受 `black` 开关管，K100 在
  T4.12。
- 样卷 PNG 不进 CI；常驻的是读帧树的那几条。

## 2026-09-03 导出引擎 M4 批次⑤b（动态页眉与公式编号）

### 一、⛔ 页眉里的 `state.at(here())` 慢一栏：动态页眉只能走 query

T4.10 的第一反应是复用 R4 那套 `state`：大题进 `section` 时 `state.update`，页眉 `state.at(here())`
取值。实测它读到的是**页眉自身位置之前**的状态 —— 大题正好起于栏顶时那一格慢一栏（实测序列
无 / 甲 / 乙 / 乙：新大题顶端那一张的页眉还挂着旧题名），而「翻页处页眉随大题切换」要的偏偏就是那
一格。

换成 `query(metadata)`：`section-header` 的左格里塞一枚 `#metadata(title)`，零尺寸惰性元素。放题名
**之前**而不是块首 —— 块首那一级会另起一个只含零尺寸元素的空段。标记的 `Location.position()` 给
`{page, x, y}`，x/y 是**页内绝对**坐标（实测：页面左边距上放的标记落在 x = `margin.left`；栏内正文
帧的 x 与 `columns` 容器栏左沿同值，与帧树 `PlacedRun.x_mm/y_mm` 同一个原点），栏号因此能从 x 反推
—— 前提是栏沿两边同源，所以 Rust 侧只留 `LayoutSpec::column_lefts_mm()` 一处算式。另两条口径顺带
钉死：x/y 是 `length`，**没有** `.round()`，只能与 `1mm` / `1pt` 比较或除以 `1pt`；页眉里的 query
看得见**后面页**的标记，算式不必为「取不到未来」留后路。

### 二、归属取「本栏覆盖最长」，不取版心上沿

原本想按落点判栏：标记 y 在版心上沿附近就是「本栏起的新大题」，否则算上一大题的延续。实测这条路直
接错 —— `section-header` 带 `above: 8pt` 与 `inset: y: 3pt`，**栏顶那条大题**的标记落在版心上沿下
方 3.8mm 处，容差一放就把它读成上一栏的尾巴。

改成覆盖长度裁决：上一条标记管到本栏首条标记（本栏没有就管到栏底），本栏每条标记管到下一条或栏底，
取覆盖最长的那条。大题起于栏顶时它独占整栏，切换因此落在正确那一格；一栏被上一个大题的尾巴占住时
保持旧题名，不会「还没讲到就先挂上题名」。一格都归不出来才落回 `fallback`（卷名），平局取先出现的
（`spans` 用严格 `>` 比较，`best` 从 `spans[0]` 起）。

### 三、typst 的列表构造，三条路各崩在一栏卷上

栏沿 `xs` 与网格轨道都必须在函数里现造，不能从 Rust 侧递逗号串：

- `array(1mm)` 是**类型转换**不是构造器，报 `expected array, bytes, or version, found length`；
- 单栏写成 `(18mm)` 只是一个 length 而不是元组，`range`/`filter` 全空；
- `repeat(2, 1fr)` 连参数都不对味，第一枚位置参数要 content，报 `expected content, found integer`。

只有 `range(n).map(i => left + step * i)` 与 `range(n).map(_ => 1fr)` 在 1 栏和 2/3 栏上都给
Array —— 三条崩路都记进了 `running-head` 的 doc，免得下次又有人手搓元组。

### 四、格按页脚的切法切，栏数按内容的口径取

轨道与 `gutter` 沿用页脚同一套（等分轨道 + 容器栏距同值），所以每格正好压在自己那一栏上方。但**栏
数不能同一个口径**：页脚走 `logical_slots()`（页码口径），页眉走 `spec.columns`（内容口径）—— A3
三栏那张纸只有一个页码格，却要有三个题名格。第 1 张仍按 T4.9 抑制（`if here().page() != 1`），卷头
页自己报名，页眉重复一遍只是脏。

### 五、公式编号在 Rust 侧数，不进 typst `counter`

任务表写的是「自定义计数器」，实际落地是 `Gen::formula_section` + `Gen::formula_no` 两枚字段在生成
时数好，把成品串塞进 `#equation("(1.2)")`。仲裁理由（记进实施计划 R10）：mitex 转不动的公式会走
`Err` 分支降级，而「降不降级」是**生成期**才成立的事实 —— 交给 typst counter 就得让一枚没排出来的
公式也吃掉一个号，一条坏公式弄坏整卷的编号。范围口径同样在生成期表达：`formula_section` 是
`Option`，进大题时置为「第几大题」并把 `formula_no` 归零，大题循环走完后置 `None`，所以卷末答案区
（不属任何大题）一条号都不出；行内公式（`display == false`）也不编号。

### 六、三轨网格：式子按内容盒居中、号贴栏边，单元里仍是行间口径

`grid(columns: (1fr, auto, 1fr), gutter: 4pt)`。式子与号不许挤同一轨（要么号跟着式子跑、要么式子被
推到边上），中间轨 `auto` 只吃式子自己的宽度，两边各一枚 `1fr` 才把号推到栏边。实测三件事：

- A4 单栏上式子中心在 x=110 而不是栏中心 105 —— 它随**所在内容盒**居中，也就是题目悬挂缩进后的那一
  盒，与题干同宽，读起来才是「这一题的式子」；号在本栏右沿内（断言取 `right-10 .. right+0.5`）。
- 号与式子同高（|Δy| < 2mm），不是浮在基线上。
- `$…$` 落进网格单元仍是**行间**口径：求和号的上限悬在正上方（x=103.5 / 106.5）而不是右上角
  （110.8 旁）。这条是「会不会被降级成行内式」的凭据，用 `horizon` 对齐的实测替代了肉眼判断。

式子长过栏宽时中间轨自己撑开、号被顶出右边界 —— 宁可撞版也不裁式子，与 M3 起就有的「溢出可见」
口径一致。

### 七、代价：每页多一次全档 query，实测与静态页眉同量级

同一份 A3 对折样卷开/关动态页眉各测编译：**123ms 对 123ms**，量不出来（首次编译 14.9s 是字体扫描，
两边同摊，不进结论）。所以没有为它做任何缓存或预计算 —— `query` 扫的是本文档的元数据流，而这套卷
的标记数量级是大题数（个位数）。

### 八、门在纸面上：期望表写死，几何一动就红

常驻门新增五条，全读帧树不读源码字符串：`running_head_labels_each_column_separately`（一栏一大题的
crafted 卷，期望表 `[["二","三"],["四","五"],["六","六"]]` 硬编码，先断 `pages.len()` 与表等长再逐格
断内容，每格 x 用 `column_lefts_mm()` 的 `rposition` 反查栏号 —— 栏宽或留白一改，第一响是「排出了 N
张与实测不符」，让人回去重测而不是静默错位）；`running_head_switches_on_the_sheet_its_section_starts`
（断页眉切换所在那张 = 正文里该大题起于的那张，两张对照）；`running_head_arguments_come_from_the_spec`
（页眉调用的六个几何参数逐一等于 spec 算出的数，等式两边同源）；`formula_numbers_run_per_section_and_skip_degraded_ones`
与 `numbered_formula_hugs_the_column_edge_and_stays_display_style`（前者断号序 `(1.1)(1.2)(1.3)(2.1)`
且降级那条不占号、卷末区零枚 `#equation`，后者就是上面那组几何读数）。

顺带修掉两条会假绿/假红的断言：`first_sheet_has_no_running_header` 原来靠卷名当 needle，改成与标题无
关的「第 1 张版心上沿之上为空、第 2 张非空」；`running_head_arguments_come_from_the_spec` 原来断
`!off.contains("running-head(")`，而 `PAGE_FURNITURE` 恒**定义**这个函数所以恒真，改成断调用
`running-head(here().page()`。

### 九、本批次已知边界

- `header_footer.header_title` 默认仍 false，三模式样卷上看不到动态页眉（不是漏画）；要看到它得在
  导出选项里勾上。
- 页眉只到「当前大题」这一层，不到题号；一栏里两个大题各占一半时取覆盖长者，平局取先出现者。
- 公式编号只对块级公式生效，且**没有**交叉引用：源数据无 label 语法，`@ref` 那一路仍空（实施计划
  §6.3 的口径）。
- `#equation` 的号是 `luma(110)` 灰，与页脚、页眉、密封线同一批受 `black` 管的灰阶，仍在 T4.12。
- 动态页眉把 `metadata` 查询撒进了每一张纸，将来若加「页眉显示当前题号」这类更密的标记，需要重测
  第七节那笔账。

## 2026-09-03 导出引擎 M4 批次⑤c（K100 单色与 docx 纸张同步）

### 一、⛔ 「全文 `luma(0%)`」不能字面执行：门控得拆成前景与底纹两枚

实施计划 §6.3 给 `PrintBlackOnly` 的原话是「全文黑」，照字面做会把大题条的 `fill: luma(236)` 底纹
一起转黑 —— 黑底黑字等于那一段内容从卷上消失。所以口径改成**分治**（R11）：两枚函数 `ink(c)` 收
前景（文字、线、点阵、公式编号、降级原文），`tint(c)` 收底纹，单色卷上前者恒 `luma(0%)`、后者恒
`luma(100%)`。这与 T4.6 早就存在的 callout 纯黑处理 `("luma(0%)", "luma(100%)")` 是同一条判断，只
不过是把散落在模板各处的硬编码灰一起纳进来。

灰阶**写进** `ink(luma(110))` 而不是从模板里删掉：彩色模式下这两枚是恒等函数，源码里的数仍然是
「这一处本来长什么样」的唯一记录，两边共用一套模板，不为单色卷另开一份。

顺带解释了一枚旧疑问：`#set text(fill: luma(0%))` 从 M3 起就在单色卷上，为什么灰还留着 —— set 规
则只管**没显式给色**的地方，而页脚、装订带、栅格线全是一处处写死的 `fill:` / `paint:`，盖过了它。

### 二、⛔ 调色板只能开在全链最前：`#set page` 的参数在写下那一刻求值

第一次把这两枚函数按「模板顺序自然位置」放进母版之后，单色卷直接编不出来。刚刚重测了一遍——把
`source.push_str(PALETTE_…)` 挪到 `g.prologue(doc)` 之后，门立即红：

```
编译失败：["unknown variable: ink"]
```

这就是 T4.7 那枚老坑（`unknown variable: page-cell`）的重演：`#set page(header:, footer:)` 的参数
在写下那一刻就求值，页眉页脚看不见其后任何 `#let`。而**最需要门控的三处灰恰恰全在那儿**——页脚格、
装订带、动态页眉。所以最终发射顺序是 `MITEX_PREAMBLE` → `PALETTE` → `PAGE_FURNITURE` → `prologue`
→ `FUNCTION_LIBRARY` → `body`，并且源码级门里加了一条顺序断言（`#let ink(c)` 必须早于
`#let page-cell`），免得下次有人觉得「放函数库旁边更整齐」。

代价写进了 `PAGE_FURNITURE` 的 doc：那三枚仍不许引用母版变量（`accent` / `body-font` 此刻不存在），
唯一的例外就是排在最前的调色板。

### 三、降级红也归 `ink()` 管：`#B00000` 在 K 版分离下近乎 0% 黑

`typeset::math::degraded()` 那串红色等宽原文（mitex 转不动时给人眼看的锚点）外面套了一层 `ink()`。
理由不是美学：红字印到 K 版上是近乎空白，那道公式在单色卷上就**消失**了。套上之后它在纯黑卷里是黑
字，等宽字体与「这是原文不是公式」的信号仍然在。

### 四、门搬到纸上：断的是编译产物的拾色集合，不是源码字符串

灰现在写在 `ink(luma(110))` 里，源码级断言会漏——那串灰恒在源码，只是不再上纸。所以新增产物级门
`monochrome_plate_carries_only_black_and_white`：同一份仿真 IR（挑灰最多的 A3 对折 + 开动态页眉）
编两次，收 SVG 里所有 `fill="#rrggbb"` / `stroke="#rrggbb"`。本次实测读数：

| 卷 | 第 1 张 fill | 第 1 张 stroke |
| --- | --- | --- |
| 单色（`PrintBlackOnly`） | `#000000` `#ffffff` | `#000000` |
| 彩色（对照） | 12 枚，含 `#1f4e79` `#b00000` `#b26a00` `#ececec` `#fcecea` `#fff5e6` `#5a5a5a` `#6e6e6e` | `#000000` `#787878` `#8c8c8c` `#aaaaaa` |

单色三张纸全部只剩黑白。四条断言里两条是防空跑的：彩色对照组自己必须非黑白，且它比单色卷**多出来**
的集合必须非空 —— 否则「两边都排成黑白」也能骗过前两条。样张探针同时加了一枚 `对折纯黑`，人眼验收
走同一个临时目录。

### 五、docx 的纸张不再是常量：`sectPr` 按 spec 现算

`A4_SECT_PR` / `a4_sect_pr` 退役，换成 `sect_pr(spec, footer_rid)`：`w:pgSz` 取 `paper.size_mm()`，
mm→twips 四舍五入，**宽>高时显式补 `w:orient="landscape"`**（Word 不会自己把页面转过来）。这条出口
第一次有了 A3 形状。

DoD 没走「XML 看起来对」——沿用 M2 那套 COM 探针并加了几何期望（`-ExpectPageMm` / `-ExpectColumns`），
真开 Word 与 WPS 各读一遍 `PageSetup`：

| 预设 | Word | WPS | 读回 |
| --- | --- | --- | --- |
| `a4_lecture` | PASS | PASS | 210×297mm / 1 栏 |
| `a4_practice` | PASS | PASS | 210×297mm / 2 栏 |
| `a3_fold_exam` | PASS | PASS | 420×297mm / 2 栏 |
| `a3_tri_exam` | PASS | PASS | 420×297mm / 3 栏 |

四跑退出码全 0。可见的行为变化有两处：docx 默认边距从「四边 2.5cm（1418tw）」变成 spec 的 22/18mm
（`top=1247` / `left=1020`），以及**教师讲义之外的 exam 模式 docx 现在是 A3 双栏**——PDF 早就这么排了，
两边从此同源。

### 六、生效左边距 = `w:left` + `w:gutter`：装订带正好有个落点

Word 这条算式与 PDF 侧「声明左边距 + 装订带」逐字对得上，所以 `Left` 能原样搬过去：A3 三栏卷
`w:left=794`（14mm）+ `w:gutter=1134`（20mm 带）= 生效 34mm。关键是 `w:left` 存**声明值**，两边各扣
一次会把带子算两遍。

`CenterFold` 在这里恒为 0——Word 的 `w:gutter` 只有「装订侧」一个概念，版心中线那条折痕带无处安放。
但那 20mm 没有丢：`column_gutter_mm()` 的下限把它抬进了 `w:cols w:space=1134`，折痕没了、总宽不亏。
`binding_gutter_mm()` 定义在 `LayoutSpec` 上而不是让 docx 自己抄 `SEALING_BAND_MM`：同一个数放进两个
模块，将来改带子宽度只会改到一边。

### 七、正文可用宽从 `9070` 变成算出来的

从前 `TEXT_TWIPS = 9070`（= 11906 − 1418×2）与 `A4_SECT_PR` 之间只有注释上一句「同源」，卷头信息表、
分值表、选项栅格、图片上限全用这个常数——分栏卷上表格按整幅版心定宽会被 Word 顶出栏外。现在全部走
`col_twips()` / `col_cm()` = `spec.column_width_mm()`：A4 单栏 210−18−18 = 174mm → **9865**，A3 对折
(420−16−16−20)/2 = 184mm → **10431**。这两个数我是先按心算写成 9846 / 10432，被 `cargo test` 纠正的
——1mm = 1440/25.4 ≈ 56.69tw 不是整数，四舍五入只能实测不能推。`MAX_IMAGE_CM = 14.0` 一并退役，图片
宽度上限改由调用方给（就是所在栏宽）。

常驻门 `tables_follow_the_column_width` 断的是「第一张表 = 整栏宽」且「没有任何表宽过所在栏」，外加
`pgSz/@w` 与 `cols/@num` 必须出自同一份 spec，而不是钉某个常数。

### 八、前端：版面区块对 Word 也放行，但只说它真做的事

`spec` 现在同传 `/export/pdf` 与 `/export/docx`（同一个 `resolve_spec`，字段级覆盖语义一致），否则调
整过的版面会只到 PDF 而不到 Word，比不同源更难解释。改动刻意小：`hasLayout` 让版面区块对两种格式都可
见，`pickFormat` 不再清空已有的 `layout`，预设仍只在第一次选到 PDF / Word 时拉。

配不上的一起讲清楚：留白样式那行 `v-if="isPdf"`，Word 区附一句「只同步纸张、边距与栏数；留白样式与密
封线是 PDF 排版专属」。ts-rs 重跑，`layout.ts` 只有 `HeaderFooter` 的注释跟着变。

### 九、本批次已知边界

- **页码口径两边差一倍**：Word 的 `w:cols` 是平衡栏且页码按**张**计（探针里 A3 对折卷 `pages=1`），
  而 R4 的 PDF 一张纸报两个逻辑页号。不抹平——那是 Word 的模型，不是漏做。
- docx 没有折痕带、没有竖排密封线（考生信息恒退化为首页横排表，T4.9 既有结论）。
- 留白样式、配色、字体三组参数仍只有 PDF 认，docx 只吃几何。
...
## 2026-09-03 导出引擎 M5 批次②（编译缓存与性能基准）

### 一、⛔ T5.3 写的「请求级 LRU（hash(请求体) → 产物）」已经在 typst 里了，它叫 comemo

任务分解要求 `compiler.rs` 自己挂一层缓存，P2 还判过「comemo 增量主要缓存字体/库层，收益有限」。
实测两头都不成立（`--release`，冷档前 `comemo::evict(0)`）：

| 同一份 IR 连编三遍 | 冷 | 同参数热 | 再冷 |
| --- | --- | --- | --- |
| 20 题卷 | 1086ms | **16ms** | 1001ms |
| 101 页大卷 | 2466ms | **100ms** | 2493ms |

`typst::compile` 的文档布局本来就是 `#[comemo::memoize]` 的，键就是「World 的输入指纹」（主源码字节
+ Library + 字体目录）—— 与「hash(请求体) → 产物」是同一层东西。热档那 16–100ms 里还含逐页 SVG
序列化（101 页 78ms），typst 真正重算的部分接近零。所以再包一层自带 LRU，存的是 comemo 刚算过的
东西，还要自己回答「同一份卷子的 PDF 出口与预览出口分别键在哪」。**P2 把结论写反了**：不是 comemo
收益有限，是在它上面再包一层收益有限。

**真正缺的是上界**。comemo 没有容量上限，而 T5.5 的预览面板 debounce 300ms——每改一次参数就净增一份
完整 `PagedDocument`（百页卷的 SVG 是 4MB 量级）。于是 T5.3 的「容量与失效策略」落成一个常量加一次
调用：`COMEMO_KEEP = 3`、`compile_paged` 每次编译后 `comemo::evict(COMEMO_KEEP)`（age 每轮 +1、命中
归零），evict 自身实测 1–6ms。取 3 对着预览的实际节奏：三模式来回切、改一处又改回来，都在范围内；
再往外就是每次请求净增一份却再也不会被读到的卷子。

结论先补进实施计划再动代码（执行纪律 3），落在 **R13**。

### 二、命中路径不许靠「大概很快」：`comemo::testing::last_was_hit`

DoD 是「同参数二次请求毫秒级」，而毫秒级有两种可能成因——命中了缓存，或者机器今天正好空。所以
dev-dependencies 里加 `comemo = { version = "0.5.1", features = ["testing"] }`，只为一枚断言：
`hot_path_hits_comemo` 冷 108–222ms（提交前重跑三轮，冷编那一头随负载浮动）→ 同参数热 **3–4ms**，并且
`last_was_hit()` 为真——命中的确实是 comemo，不是我的计时器运气好。

两个使用条件是踩出来的，不写下来一定会再踩：

- **`--test-threads=1` 是硬要求**。comemo 的缓存是进程全局的，而 `compile_paged` 每次编译都推进一轮
  evict；并行时兄弟用例会在我的两次编译之间把这条目的 age 推过上界，「第二次是不是命中」就变成掷
  硬币。同文件里另一枚 `identical_source_compiles_identically_across_evictions` 反过来——它不测时间，
  只钉「记忆化与 evict 都不许改变产物」，所以并行安全。
- **`testing` feature 会 feature-unify 进 typst 依赖的那份 comemo**，代价是整个 typst 树在
  `cargo test --release` 下重建（本机 4 核 ≈2m15s）。dev-dependency 不进 release 产物，线上无影响。

### 三、⛔ 贵的是 `grid` 本身，不是那趟 `measure`：选项行只留一种画法

T5.4 的门先按原样量了一遍，再逐题型拆账，结论与预期相反：**同一批 40 枚选项单元格，排成 `grid` 要
1025ms，排成定宽 `box` 只要 109ms**（空卷地板 117ms——调优前读数，本轮同一探针读到 91–96ms）。算下来一枚
`grid` 单元格 ≈14–22ms，是它上面那趟 `measure` 的三到五倍——R7 那套「估宽拿不准就让 typst 复量」的兜底
根本不是头号热点，栅格元素才是。

于是模板里的 `choices` 换成**一份渲染、两枚开关**：行一律画成
`par(leading: 0.7em, first-line-indent: 0em, justify: false)` + `box(width: 100% / n)` + `cs.chunks(n)`；
`probe` 只决定要不要跑 `layout(size => …)` 里那趟 4→2→1 复量阶梯（实测 **≈20–30ms/行**）；
`boxed: false` 是唯一还画 `grid` 的一侧——单元格里有表格 / 硬换行 / 并排图组 / 块级公式（塞进行内盒会
改分页与显示样式语义），或有图片（宽高比要解码才知道，是**宽度**唯一会系统性低估的一类）。

`probe` 的判据在 Rust 侧（`needs_probe`）：最宽选项估宽 ≤ **85%** × 单列预算才免量。R7 的阈值本身不带
余量，贴着阈值才真的需要复量。**这两枚开关都不是省钱的旋钮，是「已证安全」与「未证安全」的分界**，
与 T4.5 的 `keep-together` 同一个哲学。

顺带记三个 typst 语言事实，全是被编译报错原文钉出来的：code 里**没有 `%` 取模**
（`the character % is not valid in code`，要 `calc.mod`）；**整数没有 `.max()` 方法**（要 `calc.max`）；
`..cells` 是 `arguments`，取列表得 `.pos()`。第一版快路照着 Rust 的写法手搓 typst，32 枚用例整片红。

### 四、断言写反了一次：不许拿另一条渲染路径当参照物

改造后 `comfortable_options_keep_four_even_columns` 里「四列档距 × 2 == 两列档距」挂了：19.09mm 的两倍
不是实测的 34.28mm。量下去才发现**两条分支的几何本来就不等价**——`box(width: 100% / n)` 是精确分数、
铺满容器，而复量那侧的 `grid` 包在 `layout(size => …)` 里会把整行**收成内容宽**（铺满该是 38.2mm）。
错的是断言拿另一条路径当了参照物，不是代码。

处理方式记下来，因为这类错会再犯：断言换成**同一条路径内部**的恒等式，跨路径只比「首列起点 x」这一条
真正可比的量；那个收缩缺陷**本批次不修**（改的是复量那侧的版面，与性能无关），记成 follow-up #38，并在
测试注释里明确禁止再拿它当快路的参照物。顺便，三栏卷上早就存在的「四列挤在左半截、间距
12.16 / 10.94 / 11.12mm 不等距」是同一病灶的极端形态——它一直在 `layout` 那一侧，只是过去没有断言量过它。

### 五、125ms 的噪声底：这枚成本探针的对照组重做过两次

`cost_of_the_measure_fallback` 想量的是「复量一行多少钱」。第一版拿「把所有题判成单列」当对照，而单列
只留一种画法之后连版面长度都变了（兜底 3 页 对 单列 4 页，差值成了**负的** 56ms——把页数差读成了单价）；
第二版改成在仿真卷源码里翻 `probe: true`，翻到的却是**模板定义那一行的默认值**：`sim_doc` 在 a4_practice
下一枚都不复量，所有调用点都逐字传参，于是两组源码逐字相同，仍然报出差 125ms。

这一跤顺手量出了这把尺子的**噪声底**：3 页仿真卷、中位数取三，抖动 125ms（读数的 16%）。所以
`cost_of_a_preview` 那张每档只编一次的矩阵只支持读几百毫秒级的归因，更小的差一律记「读不出来」——
这条写进了两个探针的文档注释。终形态：同一份前言 + 20 行合成四选项（每枚 ≈9.4em，两种取值都排得进
四列），只翻调用点的 `probe`，**页数相等升级为断言**，5 轮 evict 冷编取中位。读数（同日四轮）：
**兜底开 1173 / 1655 / 1548 / 1289ms 对 全免量 759 / 1047 / 950 / 835ms，四组都同为 4 页 ⇒ 差
414–608ms、复量一行四选项 ≈20–30ms（每枚 5–7ms）**。跨轮看两组的绝对值同涨同落，站得住的是差值——
四轮都越过噪声底三倍，所以这一枚单价记成**区间**而不是点。

### 六、⛔ T5.4 三档读数：热请求达标，冷编译两档不达标

| 门 | 调优前 | 调优后（冷 → 再冷） | 判定 |
| --- | --- | --- | --- |
| 20 题仿真卷 ≤500ms | 混合卷 929ms；纯短选项卷 389ms | 混合卷 **674 → 648ms**；纯短选项卷 **502 → 435ms** | 短选项卷贴线 ✓；**混合卷 ✗** |
| 100 页 ≤2s | 101 页 2433–2526ms；76 页 1729ms | 101 页 **2521 → 2524ms**；76 页 **1909 → 1927ms** | 80 页内 ✓；**百页 ✗** |
| 热请求毫秒级 | 12–100ms | **14ms**（1–2 页）/ **78ms**（76 页）/ **99ms**（101 页） | ✓ |

拆开看，两个不达标是两件不同的事：

- **混合卷 674ms 超门 174ms**，而它那 10 道贴阈值的四选项题按 20–30ms/行 就是 200–300ms——比缺口还大。所以
  这一档唯一的过门办法是**把贴阈值的复量也免掉**，那正是 R7 拒绝做的事（估宽错了就少一列或多一列，
  教师看到的是排坏的卷面）。这是一笔「版面安全换 174ms」的交易，不是旋钮。
- **百页 2524ms 超门 520ms**，而这份卷子一道多列选择题都没有——两枚开关与它无关，本轮调优一分未动
  正是这个原因。它的 2.5s 全是 `400 题 × 6.4ms 内容项 + 101 页页家具（490ms）+ 91–96ms 地板`。

调完之后剩下的钱在哪：把 40 枚定宽盒里的 `1` 换成 `甲乙丙丁戊己庚辛`，同 40 盒、同 40 行，读数从 192ms
跳到 469ms；把这 320 个汉字排进普通段落、根本不进选项行，也要 +220ms。**渲染器不再是热点，CJK 逐字
shaping 才是**，而它在 typst 里面。要过门只能减每页内容项（留白默认高度、选项字号）或者换内核。

裁决已下（2026-09-03，批次收口前）：**门按重述走，版面不让**。
- **百页门采纳重述**：`≤80 页 ≤2s`（实测 1909 → 1927ms ✓）+ 百页走热请求（实测 99ms）；百页冷编 2.5s
  记为**已知不达标项**并附归因，不再挂一个做不到的常数。
- **那 174ms 不买**：贴阈值的复量保留（R7 的原意），混合 20 题卷那道 500ms 因此从「目标」降为
  **≤700ms 回归哨兵**——职责不是承诺体验，是谁将来放宽免量余量或把 `grid` 加回选项行时能立刻看见回退。
- 由此 **T5.4 按重述后的常数判定达标**；三档读数表里那两行 ✗ 保留不动，它记的是原常数下的事实。
  正文 §6.4 那句「请求级 LRU + 单卷 ≤500ms / 百页 ≤2s」加了作废指针指向 R13；任务分解表没动，
  由 R13 声明「T5.4 那行的三个常数以本条为准」。

### 七、本批次已知边界

- 免量那侧「40 枚盒 ≈8ms」这类读数**低于噪声底**，只能说「不要钱」，给不出单价。
- `boxed: false` 那一侧（含图片、表格、硬换行、块级公式的选项）与调优前逐字相同，包括那个收成内容宽
  的版面缺陷（follow-up #38）。
- 冷编译两档的门按裁决重述为「≤80 页 ≤2s + 百页走热请求」，混合卷降为回归哨兵；**百页冷编 2.5s 与
  混合卷 674ms 仍是既成事实**，将来要真过原门只剩「减每页内容项」或「换排版内核」两条路。
- `COMEMO_KEEP = 3` 是按「三模式来回切」的节奏挑的经验值，没对着真实会话调过；T5.5 预览面板上线后
  应当按实际命中率复查这一枚常量。
- `evict` 是进程全局的：多个并发请求会互相推进 age，所以高并发下有效容量比 3 小。这是可接受的
  （宁可多编一次也不要无上限涨内存），但要清楚它不是一个严格的 per-key LRU。
