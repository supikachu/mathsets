...
mod tests {
    ...
    ///
    /// **门槛口径（2026-09-03 裁决重述，全文见实施计划 §15.1 R13）**：冷编译 `≤80 页 ≤2s`
    /// （76 页实测 1927ms ✓）+ 百页走热请求（实测 99ms）；20 题**短选项**卷 ≤500ms（实测 435ms ✓）；
    /// 20 题**混合**卷按 ≤700ms **回归哨兵**盯——它那 174ms 的超出量是「保留贴阈值复量」的代价，
    /// 裁决明确不用版面安全换它。所以百页 2.5s 与混合卷 674ms 是**已接受的既成读数**，不是待修缺陷；
    /// 真要过原门只能减每页内容项或换内核。这一枚探针的职责因此是「回退报警」，不是「达标检查」。
    #[tokio::test]
    #[ignore]
